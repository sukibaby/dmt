...
int myStoi(const char *UserFlag, int MaximumValue, const char *InputArgument)
{
    int_least64_t ret;
    auto [Ptr, Ec] = std::from_chars(InputArgument, InputArgument + std::strlen(InputArgument), ret);

    if (Ec == std::errc::invalid_argument)
    {
        std::cerr << "Error: " << UserFlag << " value is out of range.\n";
        std::exit(EXIT_FAILURE);
    }

    // Assuming the user intentionally used a "as large as possible" value to disable timeouts or show all results.
    if (Ec == std::errc::result_out_of_range)
        return std::numeric_limits<int>::max();

    if (ret < 0) {
        std::cerr << "Error: " << UserFlag << " value cannot be negative.\n";
        std::exit(EXIT_FAILURE);
    }
